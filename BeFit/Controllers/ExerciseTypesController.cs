...
namespace BeFit.Controllers
{
    public class ExerciseTypesController : Controller
    {
        ...
        {
            var isUsed = await _context.Exercises.AnyAsync(e => e.ExerciseTypeId == id);
            if (isUsed)
            {
                TempData["ErrorMessage"] = "Nie można usunąć typu ćwiczenia, ponieważ jest używany w zapisanych ćwiczeniach.";
                return RedirectToAction(nameof(Index));
            }
